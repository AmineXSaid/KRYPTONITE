 ...
 *
 * roundel.js must have run first — see the same guard in sidebar.js. Both
 * scripts load via `<script src>` tags in the shell, which in a VS Code
 * webview can race: the second tag may start parsing before the first
 * finishes executing. The guard below polls for readiness rather than
 * assuming sequential execution.
 */
(function _boot() {
  if (!window.__kxRoundel) {
    setTimeout(_boot, 5);
    return;
  }
  _run();
...
(function () {
  ...
  function mount() {
    $("root").innerHTML =
      ...
      '<div id="cc">' +
        '<header class="cc-header">' + roundel(18, "", "sm") +
          ...
          '<button class="icon-btn" id="ccReload" title="Reload profiles" aria-label="Reload profiles">' +
  ...
  function renderStrip() {
    ...
    for (var i = 0; i < SECTIONS.length; i++) {
      ...
      html += '<button role="tab" data-section="' + id + '" aria-selected="' +
        ...
        '<span class="badge' + (alerts[id] ? " alert" : "") + '">' + esc(badge) + "</span></button>";
    }
  ...
  function secEndpoints() {
    ...
    if (!S.profiles.length) {
      ...
      var rows = '<div class="trow head"><span></span><span>Profile</span><span>Model &amp; wire</span><span>Status</span><span></span></div>';
    ...
    if (a) {
      ...
      html += card("Request shape", kv([
        ...
        '<div class="muted" style="font-size:11px;margin-top:5px">' + esc(PROFILE_TIP) + "</div></div>");
  ...
  function secAuth() {
    ...
    html += '<div class="grid-cards">' +
      ...
      card("Secret resolution",
        '<div class="tbl"><div class="trow2 head">' +
  ...
  function secretRows(p) {
    ...
    if (!refs.length) {
      return '<div class="trow2"><span class="empty">No interpolated references.</span><span></span><span></span></div>';
    }
    var html = "";
    for (var i = 0; i < refs.length; i++) {
      html += '<div class="trow2"><span class="v mono ell">' + esc(refs[i][0]) + "</span>" +
  ...
  function secTls() {
    ...
    html += '<div class="grid-cards">' +
      card("CA bundles", (caRows || '<span class="empty">Node&rsquo;s bundled root store.</span>') +
        (a.tls.ca.length > 1
          ? '<div class="muted" style="font-size:11px;margin-top:5px">' + a.tls.ca.length + " bundles combined at request time.</div>"
          : "")) +
  ...
  function certChain(p) {
    ...
    var rows = '<div class="trow3 head"><span>#</span><span>Subject</span><span>Expiry</span></div>';
    for (var i = 0; i < chain.length; i++) {
      var isRoot = i === chain.length - 1;
      var expiry = i === 0 ? expires : isRoot ? "in trust store" : "—";
      var cls = S.tlsError && isRoot ? ' style="color:var(--red-400)"' : "";
      rows += '<div class="trow3"><span class="muted">' + (i + 1) + "</span>" +
  ...
  function secDiag() {
    ...
    if (S.tlsError) {
      html += '<div class="card" style="margin-top:14px;border-color:var(--red-400)">' +
        '<div class="t" style="color:var(--red-400)">' + icon("i-x", "ic-13") +
  ...
  function secAgent() {
    ...
    var rows = '<div class="trow3 head" style="grid-template-columns:minmax(0,1fr) minmax(0,1.4fr) minmax(0,1fr)">' +
      "<span>Tool</span><span>Arguments</span><span>Sandbox</span></div>";
    for (var i = 0; i < TOOLS.length; i++) {
      rows += '<div class="trow3" style="grid-template-columns:minmax(0,1fr) minmax(0,1.4fr) minmax(0,1fr)">' +
        '<span class="mono ell">' + esc(TOOLS[i][0]) + "</span>" +
  ...
  function secSkills() {
    ...
    if (!S.skills.length) {
      ...
      for (var i = 0; i < S.skills.length; i++) {
        var s = S.skills[i];
        rows += '<div class="trow4">' +
          '<button class="skill-row" data-skill="' + esc(s.name) + '" role="checkbox" aria-checked="' +
          (s.enabled ? "true" : "false") + '">' +
          '<span class="skill-cbx">' + (s.enabled ? icon("i-check", "ic-9") : "") + '</span>' +
          '<span class="ell" style="font-size:12px;color:var(--cream)">' + esc(s.name) + "</span></button>" +
  ...
  function secCheckpoints() {
    ...
    var rows = '<div class="trow4 head"><span>Label</span><span>When</span><span>Hash</span><span></span></div>';
    for (var i = 0; i < S.checkpoints.length; i++) {
      ...
      rows += '<div class="trow4">' +
        ...
        '<span class="mono ell">' + esc(c.hash.slice(0, 10)) + "</span>" +
  ...
  function secLogs() {
    var html = "<h3>Logs &amp; export</h3>" +
      '<div class="explainer">The last 200 lines from the GENESIS output channel.</div>' +
