...
/* The webview names the product in its welcome masthead - the mark, then
   GENESIS in tracked mono, above the terminal frame. Lower-case in the markup
   because the CSS uppercases it, which is how a wordmark is set. */
ok("and the webview, in its welcome masthead",
  /boot-word">genesis/.test(read("media/webview/sidebar.js")));
