...
/* The webview's wordmark is gone - the welcome is a terminal now and names the
   product in the command it runs, which between the tab strip and the report's
   own `workspace` row was already saying it twice. Lower-case on purpose: it is
   a command name, and a shell does not capitalise one. */
ok("and the webview, in the command its welcome runs",
  /boot-cmd">genesis init</.test(read("media/webview/sidebar.js")));
