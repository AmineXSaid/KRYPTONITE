 ...
 *
 * Run: npx esbuild test/recall-state.ts --bundle --outfile=dist/recall-state.cjs \
 *        --format=cjs --platform=node --target=node20 && node dist/recall-state.cjs
 */
import * as fs from "node:fs";
import * as os from "node:os";
import * as path from "node:path";
import { RecallStore, dedupeKey, type ProposalInput } from "../src/recall/state";
...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "kx-recall-"));
let n = 0;
/** A fresh store on its own storage root, so suites cannot leak into each other. */
function store(root = "/w/one", zone = "default"): RecallStore {
  const dir = path.join(tmp, `s${n++}`);
  fs.mkdirSync(dir, { recursive: true });
  return new RecallStore({ globalStorageUri: { fsPath: dir } as any }, root, zone);
}
...
(async () => {
  ...
  fs.mkdirSync(path.join(broken, "recall"), { recursive: true });
  const bs = new RecallStore({ globalStorageUri: { fsPath: broken } as any }, "/w/b");
  // Reach the file the same way the store does, then corrupt it.
  await bs.flush();
  const bdir = fs.readdirSync(path.join(broken, "recall"))[0];
  if (bdir) {
    fs.writeFileSync(path.join(broken, "recall", bdir, "state.json"), "{not json", "utf8");
    const after = new RecallStore({ globalStorageUri: { fsPath: broken } as any }, "/w/b");
