...
console.log("\n──── where it lands in the prompt ────");
{
  ...
  const withIt = systemPromptFor([], "act", instr);
  ...
  ck(systemPromptFor([], "act", instr) === withIt, "and the same inputs give the same string");

  const planned = systemPromptFor([], "plan", instr);
  ...
  ck(systemPromptFor([], "act", undefined) === bare, "undefined is the same as not passing it");
