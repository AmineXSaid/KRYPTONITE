...
(async () => {
  ...
  /* ── 4c. a nonsense context window ────────────────────────────────────
   *
   * TWO GUARDS, AT TWO DEPTHS, AND BOTH ARE WORTH HAVING.
   *
   ...
   * nothing to report and no error saying why.
   *
   * The profile loader now refuses that value outright, which is the better
   * place to catch it - the user learns which field in which file is wrong,
   * rather than watching the agent behave strangely. But the loop keeps its
   * own guard, because a window reaching it is not always one a user typed:
   * `${env:…}` interpolation, a transform, and a future caller that builds a
   * profile in memory all bypass the parse. A budget computed from a nonsense
   * number is worse than no budget, so the step cap governs alone. */
  console.log("\n──── a profile with a broken window ────");
  {
    ...
    // The outer guard: it never gets as far as the loop.
    let refused = "";
    try { loadProfile(zero); } catch (e: any) { refused = String(e.message ?? e); }
    ck(refused !== "", "a zero window is refused when the profile loads");
    ck(/contextWindow/.test(refused), "and the message names the field", refused);

    // The inner guard, driven with the window forced past the loader the way
    // interpolation or a transform could. The reply has to make tool calls:
    // the budget is checked at the bottom of a step, which a turn that answers
    // immediately never reaches.
    const profile = loadProfile(file);
    (profile.capabilities as any).contextWindow = 0;
    reply = { call: { name: "list_files", args: { path: "." } } };
    usage = { input: 10, output: 5 };
    served = 0;
    const client = new EndpointClient(profile, () => undefined, tmp);
