...
/* ── the segment ────────────────────────────────────────────────────────── */
{
  ...
  ok("and the banner is hidden while acting", d.getElementById("planBanner").hidden);

  send(state("ask"));
  ok("Ask can be selected", on("ask") && !on("act"));
  const banner = d.getElementById("planBanner");
  ...
  ok("and what that promises", /nothing is edited/i.test(banner.textContent));
  ok("carrying the phase for styling", banner.getAttribute("data-phase") === "ask");

  send(state("plan"));
  ok("Plan still works", on("plan") && !d.getElementById("planBanner").hidden);
  ok("with its own wording", /Plan phase/.test(d.getElementById("planBanner").textContent));
