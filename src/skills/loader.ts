...
export function skillIndex(skills: Skill[]): string {
  ...
  return [
    ...
    "",
    "Rules:",
    "- Match the task against a description, not a name. If one applies, call",
    "  read_skill with its name BEFORE doing the work, and follow what it says.",
    ...
    "- If no description matches, do not call read_skill at all.",
  ].join("\n");
}
