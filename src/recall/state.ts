 ...
 *
 * Genesis has one kind of memory today and it belongs to the agent: a markdown
 * file the agent writes with its ordinary tools, capped, and read into that
 * agent's own prompt. It is hand-shaped and deliberate, which is its virtue.
 * What it is not is a place for the things the *user* said - the deployment
 * layout explained on Monday and explained again on Tuesday, because nobody on
 * either side was going to write it down. Every conversation still starts from
 * the same fixed head. This is where that stops.
 *
 * Named `recall` rather than after the harness these ideas came from: a
 * `src/hermes/` directory sitting beside an agent loader that cites Hermes
 * Agent as a config format guarantees that a future reader connects two
 * unrelated things. The config compatibility in `src/agents/loader.ts` and this
 * store share nothing but an influence.
 *
...
export type RecallScope = "user" | "workspace" | "global";
 ...
 */
export type RecallKind = "preference" | "fact" | "skill";
...
export class RecallStore {
  ...
  private records = new Map<string, RecallRecord>();
  ...
  constructor(
    ...
    const key = crypto
      ...
      .digest("hex")
      .slice(0, 12);
    this.dir = path.join(context.globalStorageUri.fsPath, "recall", key);
   ...
   */
  propose(input: ProposalInput): RecallRecord {
    this.hydrate();
    const now = Date.now();
   ...
   * one out.
   */
  approved(scope?: RecallScope): RecallRecord[] {
    this.hydrate();
