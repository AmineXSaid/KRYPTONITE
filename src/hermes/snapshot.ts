import type { HermesRecord, HermesState } from "./state";
...
function line(rec: HermesRecord): string {
 ...
 * exists to prevent.
 */
export function buildSnapshot(records: HermesRecord[], cap = MEMORY_CAP): MemorySnapshot {
 ...
 */
export function snapshotFor(state: HermesState, cap = MEMORY_CAP): MemorySnapshot {
  try {
